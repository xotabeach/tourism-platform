...
Для исполнимого порядка работ, контрактов, quality gates и release criteria
см. [единый implementation blueprint](implementation-blueprint-2026-08.md) и
[ADR-010 по 2ГИС](decisions/ADR-010-2gis-routing-and-map-provider.md).
...
- **Геометрия дорог — детерминированная, P2.** Какая дорога есть и
  сколько по ней ехать, решает выбранный provider (в первом test-contour —
  2ГИС Routing API, позднее возможен self-host OSRM). Контракт
  `RoutingProvider` уже написан, stub уже за портом; замена — это adapter
  плюс quality gate. Пока не сделано, любые данные о состоянии дорог
