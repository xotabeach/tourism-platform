...
- **Expert** — администраторский статус `users.is_expert`, синхронизированный с
  выделенной записью `travel_ranks` (`slug=expert`, ранг недостижим через очки).
  При выдаче назначается ранг «Эксперт», при отзыве выбирается обычный ранг по
  `travel_points`; автосинхронизация очков не демонтирует эксперта. Лидерборд
  исключает пользователей по `rank_id`, а не отдельным boolean-фильтром.
  Фактический переход статуса из формы или bulk-action админки создаёт аудит
  и inbox/push-уведомление.
