...
Регистрация требует токен из UI GitLab (Settings → CI/CD → Runners), выдать его может
только владелец репозитория. Уровень группы `travel-platform2` предпочтительнее
проектного: один раннер обслуживает все четыре репозитория.
