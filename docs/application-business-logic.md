...
Лимиты Travel+ согласованы с
[ai-route-chat-product-contract.md](ai-route-chat-product-contract.md)
и реализуются через `EntitlementService` / `QuotaPolicy`.
...
Отзывы к **маршрутам и локациям** (текст + оценка 1–5) реализованы с
модерацией. Это разные сущности (`route_reviews` и `place_reviews`) и разные
API; в админке целевой объект всегда явно подписан.
...
- `GET /places/{id}/reviews` применяет те же правила публикации, фотографий,
  ответов и среднего рейтинга к локации. Один корневой отзыв пользователя на
  локацию обновляется до публикации; собственный отзыв закрепляется в mobile,
  а composer скрывается.

Уведомления о модерации/ответах сейчас реализованы для route reviews. Аналог
для place reviews остаётся ближайшей задачей.
...
Таб «Избранное» и экран «Моё избранное» сейчас показывают маршруты, места,
follows и placeholder истории — не очередь черновиков. В первых трёх разделах
короткий swipe фиксирует действие «Убрать», а длинный swipe удаляет после 72%
ширины; snackbar позволяет вернуть объект. Editorial по-прежнему публикует
редакция.
...
### Временный beta override (as-built с 2026-09-16)

До подключения проверенного store/server billing ИИ-подбор открыт всем
авторизованным пользователям. Для free и Travel+ одинаково действуют:

- до **5 успешных генераций маршрута в сутки** (UTC);
- до **30 ответов ИИ в сутки** (UTC), суммарно по всем чатам;
- до **12 точек**, до **3 вариантов** и расширенные параметры подбора.

Travel+ показывается как «Скоро». Публичная самоактивация и `mock_checkout`
закрыты во всех окружениях; существующие подписки и admin-grant сохраняются.

**POST_BETA:** перед коммерческим запуском обязательно убрать этот override,
вернуть согласованное разделение free/Travel+, подключить проверку платежей на
backend и актуализировать paywall. Спецификация решения:
[`spec-beta-ai-access.md`](../spec-beta-ai-access.md).

### Целевая матрица после beta

Канон для подбора/генерации после коммерческого запуска (согласован с paywall Figma,
...
- стандартное начисление тп.

## 16. Возможности Travel+
...
- расширенный offline избранного (остаток Phase 12).
