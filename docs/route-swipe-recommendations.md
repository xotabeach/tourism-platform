...
- Простой, объяснимый скоринг на уже существующих типизированных
  колонках — без эмбеддингов и ML-обучения (это уже P1/P4 в
...
```text
score =
    0.45 * category_affinity   +   -- пересечение категорий маршрута
                                    -- (через его stops → places) с
                                    -- категориями избранных маршрутов
                                    -- и мест пользователя
    0.20 * region_affinity     +   -- 1.0 если регион маршрута совпадает
                                    -- с самым частым регионом среди
                                    -- избранного пользователя, иначе 0
    0.20 * popularity          +   -- log(1 + count(favorite_routes
...
**Холодный старт** (у пользователя 0 избранного и 0 пропусков):
`category_affinity` и `region_affinity` не определены → вес
перераспределяется на `popularity`/`freshness` (просто самые
популярные + немного свежих, разнообразные по региону).
