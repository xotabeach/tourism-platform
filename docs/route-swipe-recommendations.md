...
Статус: **backend v1 и mobile R2 сделаны 2026-08-29.** Серверная колода,
skip feedback и ranker v1 готовы; приложение читает `GET
/routes/recommendations/today`, показывает причину рекомендации на карточке и
отправляет skip с `client_event_id`. Остаются reset/opt-out персонализации,
host cron колоды и «Почему это?» как отдельный explain-экран.
...
skip feedback и ленивая генерация. **Mobile R2 (2026-08-29) подключён:**
колода приходит из API (`4fddf6d`), карточка показывает причину
рекомендации по `explanation_code` (`cc5a6e2`), свайп влево шлёт
idempotent skip. Host cron на 03:00 МСК не установлен; без него колода
считается при первом запросе за день.
