...
1. **Контент для публикации** — гейт готов (`/admin` → Места), но проходят
   его лишь **144** места из 5000: 4824 блокирует отсутствие
   содержательного описания. Медиана длины OSM-описания — 9 символов.
   Нужен `enrich_places_content --llm` + редакционная проверка, иначе
   `place_picker` так и будет видеть почти только seed-места.
...
- **Замер:** готовы 144 места, 4824 блокирует отсутствие описания, 700 —
  отсутствие города. Медиана длины OSM-`description` = 9 символов
  (маркеры «МК-29»), поэтому 786 непустых дают лишь 144 пригодных.
  Вывод: настоящий блокер — контент, а не гейт.
