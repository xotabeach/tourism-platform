...
## Update — 2026-08-30 (ночь, продолжение): фикс ранкера, демо-маршруты, APK 0.2.0

- **Настоящая причина короткой колоды рекомендаций у активного юзера
  найдена и исправлена.** Не cron, не catalog fallback — региональный
  cap диверсификации (`ranker.py::_respects_caps`, `MAX_REGION_SHARE=0.5`)
  резал колоду вдвое **независимо от числа кандидатов**, потому что весь
  каталог лежит в одном регионе (Крым). Cap теперь применяется только когда
  регионов больше одного. После фикса + очистки избранного у пользователя
  колода выросла с 7–8 до **16 карточек**.
- **12 демо-маршрутов** добавлены от лица существующего эксперт-аккаунта
  («Борода»), не выдуманного пользователя — `scripts/seed_demo_routes.py`,
  разные по сложности/транспорту/сезону/числу точек, все из уже
  опубликованных мест и в пределах 50 км лимита demo-ключа 2ГИС. Геометрия
  посчитана и применена (`backfill_route_geometry.py`).
- **План на 2026-08-31:** автотриггер пересчёта геометрии при
  создании/правке маршрута + крон-актуализация устаревшего —
  `docs/route-geometry-autorefresh-plan-2026-08-31.md`. Не реализовано,
  только спроектировано (очередь + одиночный consumer с троттлингом
  5 req/min, отдельно от circuit breaker HTTP-процесса).
- **APK 0.2.0 (build 18)** собран: `production` + `api` (тестовый сервер —
  отдельного прод-API по-прежнему нет), `--split-per-abi` (arm64 для
  Xiaomi Redmi) + универсальный. Версия поднята с 0.1.16 на 0.2.0 —
  накопился большой пакет изменений (баллы за прохождение, карточка
  достижения, in-app уведомления вместо SnackBar, 14 новых разделов
  админки, фикс колоды).

## Update — 2026-08-30 (ночь): баллы, достижения, уведомления, админка

- **Пустая колода рекомендаций — причина найдена и починена.** Это был не
  отсутствующий cron: у активного юзера в каталоге из 27 маршрутов было
  **22 в избранном + 8 скипов**, а ранкер исключает favorites/skips/completed
  (`ranker.py:86`) — кандидатов не оставалось. Теперь при пустой колоде
  `get_today_deck` добирает карточки из обычного каталога с явным
  `explanation_code=catalog_fallback` (скипы и пройденное по-прежнему
  исключаются — это явный сигнал пользователя). Добавлен
  `POST /routes/recommendations/refresh`. Cron колод установлен на хосте
  (00:40 UTC, wrapper + flock по образцу retention).
- **Баллы за прохождение (`rewards.py`, миграция `0045`).** Не константа:
  считается от **immutable снапшота старта** — дистанция (пешком дороже
  за км), набор высоты, бонус за уклон >20°, множитель сложности, всё
  масштабируется числом реально отмеченных остановок. Старт-и-сразу-финиш
  даёт только базу. `awarded_points` на прохождении — и результат, и guard
  идемпотентности (повторный complete не платит дважды).
- **Достижения:** миграция `0046` добавила `icon_slug` и `how_to_earn`
  (backfill из `description`, где и лежало правило). Новый экран карточки
  достижения в приложении: иконка, название, статус с точной датой/временем
  получения, описание и «как получить». Экран достижений и профиль больше
  не отвечают снекбаром.
- **Все уведомления переведены на in-app.** Единый
  `core/design/components/app_notice.dart` вместо `SnackBar` в **20 файлах**.
  Undo-действия сохранены (компонент принимает action и перехватывает тапы
  только когда оно есть). Таймер автоскрытия живёт в State — иначе висел
  после уничтожения дерева, это ловили существующие тесты избранного.
- **Свайпер:** кнопка фильтра больше не дублирует чипы под поиском — там
  теперь переключатель «Показывать рекомендации» и «Обновить рекомендации»,
  в том же «пузыре», что и меню маршрута. Смена фильтра анимируется
  (`AnimatedSwitcher` с ключом по фильтру/источнику).
- **Админка:** было 13 разделов, 33 таблицы без интерфейса. Добавлены
  достижения и прохождения, колоды и реакции рекомендаций, география и
  категории, дорожные события, уведомления и медиа — 14 новых разделов
  (все открываются, проверено). Производные данные read-only, токены
  устройств маскируются. Sidebar переведён на нейтральный тёмный с
  фирменным зелёным акцентом, скроллится внутри себя.
- **Pull-to-refresh профиля** теперь инвалидирует и достижения (раньше
  обновлялся только сам профиль).

## Update — 2026-08-30 (вечер): карта, точки, geometry backfill

- **Лимиты demo-ключа 2ГИС выяснены точно** (личный кабинет + сырые ответы):
  **5 запросов Routing API в минуту**, при превышении ключ временно
  блокируется; и **жёсткий потолок 50 км** на маршрут. Ранее наблюдавшиеся
  «ConnectTimeout с VPS» — это следствие блокировки, а не сетевая авария.
  Backfill теперь выдерживает паузу 15с между вызовами провайдера.
- **Ошибка маппинга исправлена:** 403 «excessive distance ... max (km): 50»
  сваливался в общий `routing_provider_error`, из-за чего лимит тарифа
  выглядел как отказ провайдера (8 маршрутов в отчёте). Теперь это
  `route_too_long` — и в отчётах, и для пользователя.
- **Geometry backfill выполнен: 14 из 27** опубликованных маршрутов переведены
  на дорожную геометрию (помечены `backfilled`). Остальные 13 **невозможно**
  сконвертировать на demo-ключе: все длиннее 50 км, кроме одного
  (`routing_unreachable`). Вернуться к ним только с платной подпиской.
  VPS не может считать геометрию сам, поэтому скрипт получил `--export` /
  `--input`: расчёт на машине с доступом к 2ГИС → применение рядом с БД.
  Применение коммитит по маршруту, чтобы обрыв не терял посчитанное.
- **Свои точки вместо точек 2ГИС.** Backend получает явные `center`/`zoom` и
  рисует только подложку и линию (`pins=none`); приложение ставит свои
  кликабельные пины поверх растра по той же Web Mercator математике
  (проверено по реальным пиксельным смещениям, есть тест `map_projection`).
  Раньше на одном холсте были два несовпадающих набора точек.
  Тап по точке — карточка с фото и кратким описанием (новые поля
  `place_cover_url` / `place_short_description` в `RouteStopOut`), тап по
  карте — полноэкранный зум. Карта появилась и на экране прохождения.
- **Баг, ломавший карту целиком:** в новом коде центр отдавался как
  `lng,lat`, тогда как 2ГИС ждёт `lat,lng` — каждый запрос карты возвращал
  502, приложение падало в стилизованный фолбэк (и потому «не раскрывалось»
  по тапу). Исправлено, покрыто тестом.
- **Mobile:** «Пройти маршрут» показывал общее «Не удалось загрузить данные» —
  настоящая причина вычислялась, но не передавалась во вьюху; экран был
  тупиком при активном другом маршруте. Теперь называет блокирующий маршрут и
  даёт в него перейти. Меню маршрута переехало из bottom sheet в «пузырь»,
  раскрывающийся из круглой кнопки. Список скачанных маршрутов —
  `DraggableScrollableSheet` (72%→92%) с карточками и удалением по одному.
- **Инцидент:** прод лёг — Postgres в краш-цикле, `No space left on device`,
  диск 100%. Причина: 24 старых backend-образа (7.5 ГБ) от частых деплоев за
  день. Почищены (оставлены текущий и предыдущий для отката), освободилось
  6.4 ГБ, диск 55%, данные целы. **Вывод на будущее: чистить образы на хосте
  после серии деплоев** — 15 ГБ диска не переживают много итераций подряд.
- **APK 0.1.16 (build 17)** собран: `production` + `api`,
  `dist/CrimeaTrip-0.1.16-production-Xiaomi-Redmi-arm64.apk` (arm64) и
  универсальный. Внимание: `APP_ENV=production` указывает на тестовый
  сервер — отдельного прод-API нет, и приложение при этом называется
  «КРЫМТРИП» без пометки теста.
...
Источники: [backend/AI-ревью](reviews/2026-08-25-backend-architecture-ai-review.md),
[mobile UI/UX-ревью](reviews/2026-08-25-mobile-ux-review.md),
[mobile код/security/perf-ревью](reviews/2026-08-25-mobile-code-security-perf-review.md).

Смержено (обе валидации зелёные — backend `ecb1993`: 394 теста, покрытие
74.65%; mobile `0e69467`: 245 тестов + 25 goldens):

- **Фаза 0 — backend security:** mock Travel+ самоактивация закрыта вне
  local/test, row lock на ротации refresh-токена и на квоте генерации,
  publication-фильтр на обложках и избранных маршрутах, флагнутые
  injection/crisis-реплики больше не возвращаются в контекст LLM.
- **Фаза 1 — mobile:** картинки в ИИ-чате идут через media allowlist.
- **Фаза 2 — mobile:** `homePlacesProvider` в карте инвалидации, шапка
  остаётся на месте и при загрузке, и при ошибке.
- **Фаза 3 — backend:** семафор на один GPU-слот LM Studio, метрики на
  каждый turn, `constraint_patch` не перезаписывает подтверждённые поля,
  `ST_DWithin`+`LIMIT` на geo-инструменте, обрезка истории в SQL.
- **Фаза 4 — mobile:** узкие `.select` на сессию, push-bootstrap после
  первого кадра, меньше пересборок в свайп-колоде.
- **Фаза 5 — mobile:** пайплайн ИИ-чата вынесен в `RouteMatchNotifier`
  (+ behavioral-тесты, которых раньше не было вообще).
...
Архитектурный вердикт (монолит/микросервисы, поднят и ревью, и отдельным
разбором независимо): ничего выносить рано — см. план, раздел
«Explicitly not now».
...
**Остаток P0-bis, детали и цифры в
[роадмапе](route-intelligence-roadmap.md) §P0-bis:**

1. ~~**Дедупликация seed ↔ OSM**~~ — выполнено: OSM-дубли сливаются в
   seed-места до публикации.
2. ~~**Описания из Wikipedia**~~ — выполнено для доступных
   `wikipedia`/`wikidata` extracts; LLM остаётся только на остаток и через
   review.
...
- **P1 — семантика:** заменить `hash-v1` (bag-of-tokens хеш, не
  семантика) на настоящий эмбеддер, распространить вектора на
  `places` / `routes`, гибридный retrieve.
- **P2 — OSRM:** сейчас маршрут это прямая × 1.35; для серпантинов
  Крыма расхождение кратное.
- **CI-минуты:** дать `tourism-mobile` / `tourism-platform` /
...
- ~~**Точка входа «Новый чат»** → отдельный экран истории чатов, вход из
  настроек (вариант 3).~~ Сделано, в `main` с 2026-08-26 —
  `ChatHistoryScreen` + `RouteMatchNotifier.resumeSession`. Логика
  создания новой сессии (`startNewChat`) живёт там же, в notifier.
...
- Backend: «Эксперт» теперь отдельная admin-only запись в `travel_ranks`;
  выдача/отзыв статуса синхронизирует `rank_id`, а лидерборд исключает этот
  ранг. Очки не могут автоматически демонтировать эксперта.
- Backend/mobile: «Сменить предпочтения» стало рабочим тестом. Ответы
  (`preferred_categories`, `preferred_difficulty`, `travels_with_kids`,
  `travels_with_pets`) сохраняются через `PATCH /me/preferences`.
- Backend/mobile: обращения в поддержку принимают до трёх фотографий;
  сервер проверяет владельца, удаляет EXIF и сохраняет WebP. В форме ошибки
  устройство и версия приложения подставляются автоматически.
- Mobile: поиск добавлен на полный список «Маршруты / Локации», фильтры
  результатов подбора и избранного работают; AI-карточка получила единый
...
Фазы ремедиации лежали на семи ветках (2 backend + 5 mobile + доки), ничего
не было смержено. Плюс рабочая сессия 2026-08-25 (тумблер Маршруты/Локации,
`AllListScreen`, `ChatHistoryScreen`, фиксы hero-шапки, конечная свайп-колода)
осталась незакоммиченной и была отложена в stash, когда нарезались ветки —
восстановлена в `feat/home-modes-and-chat-history` (`62885c0`).

Backend смержился fast-forward. Mobile потребовал ручного разрешения:
каждая ветка ветвилась от `main` независимо, и восстановленная сессия
трогала те же файлы. Существенные решения:

- Фаза 2 сделала `homePlacesProvider` алиасом `placesListProvider` — только
  потому, что настоящего провайдера на её базе не существовало. Оставлен
  настоящий.
- Фаза 4 (узкий `.select` на сессию) выиграла у широкой подписки; шапка
  Главной теперь принимает `name`/`avatarUrl` параметрами.
- Фаза 5: resume сессии переехал в `RouteMatchNotifier` — туда, где теперь
  живёт весь пайплайн чата, а не в `State` экрана. Добавлены 2 теста.
- Фаза 6 ветвилась **до** вчерашних фиксов hero-шапки, поэтому её общий
  компонент унёс в себя **до-фиксовую** «губу» (та, что масштабировалась и
  отрывалась при скролле). Оставлен исправленный `CollapsingSheetLip`;
  общие `EntityReviewsSection` / `AudioGuideCard` приняты целиком.

**Долг, всплывший при мердже:** `EntityReviewsSection` вынесен из
до-токенной версии файла отзывов, поэтому унёс с собой захардкоженные
цвета и радиусы — вчерашний проход по токенам в этом файле не пережил
унификацию. Внесено в фазу 8 плана.

Валидация после мерджа: backend `ecb1993` — 394 теста, покрытие 74.65%;
mobile `0e69467` — 245 тестов + 25 goldens, format/analyze чисты.

Документация приведена в соответствие с фактическим состоянием `main`,
добавлена бизнес-спецификация экранов
([mobile-screens-business-spec.md](mobile-screens-business-spec.md)) —
описание каждого экрана на продуктовом языке, с явной пометкой того, что
ещё не реализовано, для прохода дизайнера в роли тестировщика/БА.

### 2026-08-25 (5) — Второе mobile-ревью (код/security/perf, Cursor) — влито в план

Дал Cursor отдельную инструкцию на mobile: код-архитектура, реализация
UI/UX, безопасность, производительность — [reviews/2026-08-25-mobile-code-security-perf-review.md](reviews/2026-08-25-mobile-code-security-perf-review.md).
Вышло глубже узкого UX-ревью (4): не повторил его находки, копнул туда, где
я не смотрел. Два самых важных пункта перепроверил вручную перед тем как
вносить в план — оба подтвердились дословно:

- **SEC-1/PERF-1** — во всём `lib/` только 3 места используют голый
  `Image.network` вместо `AppImages.coverImage`/`resolveMediaUrl` — и все
  три в чате с ИИ (`chat_place_chip.dart:41`,
  `chat_route_proposal_card.dart:241,940`). Это единственная брешь в
  media-allowlist во всём приложении, и `test/security` её не ловит — тест
  проверяет саму allowlist-функцию, а не эти конкретные call-site'ы.
- **CODE-1** — `homePlacesProvider` вообще отсутствует в карте инвалидации
  `app_data_refresh.dart` (строки 116-118: scope `home` инвалидирует только
  `homeRoutesProvider`/`topTravelersProvider`) — pull-to-refresh на
  Локациях, и даже `AppDataRefreshScope.all`, не обновляют данные никогда.

Плюс: `route_match_screen.dart` (1406 строк, ~30 полей состояния) без
Notifier и без единого behavioral-теста на chat pipeline/Travel+ activate
(только layout/golden/safety-keyword тесты); place↔route «twin» разошлись
не только токенами (уже знали), но и структурно — reviews, аудио-карточка,
collapse-параметры существуют как две параллельные копии, а не общий код;
swipe-колода пересобирает до 3 тяжёлых карточек на каждый кадр анимации;
Firebase bootstrap блокирует `runApp`.

Переупорядочил [reviews/2026-08-25-review-remediation-plan.md](reviews/2026-08-25-review-remediation-plan.md)
по итогу — с backend-ревью (4) пересечений почти нет (разные слои), с
mobile UX-ревью (4) — прямое дополнение, слил в объединённые фазы (0-8)
вместо separate-трека.

### 2026-08-25 (4) — Три ревью проекта (backend/AI, mobile UI/UX, архитектура) + план ремедиации

Пользователь попросил полный разбор проекта (техника, UI/UX, связь с
локальным ИИ, архитектура/микросервисы) в преддверии доступа к Fable 5;
поскольку доступ пока не открыт, сделали то, что можно было сейчас:

- **Backend + AI** — отдельное ревью через Cursor,
  [reviews/2026-08-25-backend-architecture-ai-review.md](reviews/2026-08-25-backend-architecture-ai-review.md).
  Самое важное: 4 P1 security/correctness бага (mock Travel+
  self-activate в любом env, refresh-rotation без row lock, гонка квоты
  генерации, favorites/cover media без publication-фильтра) и подробный
  разбор AI-пути (LM Studio: нет concurrency-лимита на один GPU, нет
  метрик fallback/latency, `topic_guard` только keyword-based, история режется
  `[-12:]` без токен-бюджета, RAG всё ещё `hash-v1` и выключен по
  умолчанию).
- **Mobile UI/UX** — свой аудит (фок-агент) вне уже разобранных сегодня
  экранов, [reviews/2026-08-25-mobile-ux-review.md](reviews/2026-08-25-mobile-ux-review.md).
  Нашёл повтор бага «полноэкранный блик на загрузке» (тот же класс, что
  чинили на Локациях) на `my_routes_screen.dart`, там же — сырой текст
  исключения в UI при ошибке; `SearchScreen` похож на мёртвый код без
  единой точки входа.
- **Архитектура/микросервисы** — отдельный вопрос (свой фок-агент),
  вывод независимо совпал с архитектурным разделом backend-ревью: сейчас
  ничего выносить не стоит (ADR-001 ещё держится), `route_builder`
  (AI-часть) стоит трактовать как формализованную внешнюю зависимость
  (health-check, семафор, метрики), а не выносить в отдельный сервис —
  инференс и так уже физически на другой машине.
- Всё сведено в единый бэклог —
  [reviews/2026-08-25-review-remediation-plan.md](reviews/2026-08-25-review-remediation-plan.md),
  фазы по приоритету (P1 security → mobile-баг того же класса, что и
  сегодняшний фикс Home → усиление границы LM Studio → уборка). `/code-review
  ultra` по обоим репозиториям запущен пользователем параллельно — находки
  оттуда ещё предстоит влить в план.

### 2026-08-25 (3) — Свайп-назад в истории чатов; Локации на Главной: лимит + скелетон; release APK

- **Свайп-назад ломался на двух новых экранах из (2).** `ChatHistoryScreen`
  был top-level route на `CustomTransitionPage` (без нативного iOS-жеста) —
  перенесён внутрь дерева настроек как `CupertinoPage`-роут (как
  `settingsAccount` и другие), нативный edge-swipe заработал сам собой.
  Возобновлённый чат (`RouteMatchScreen(resumeSession: …)`) свайпом уходил
  на Главную вместо возврата к списку: `_goBack()` был хардкожен на
  `context.go('/')` — верно для таб-корня (там нет Navigator-маршрута под
  собой), неверно для запушенного из истории инстанса. Теперь при
  `resumeSession != null` жмёт `context.pop()`.
- Попутно всплыл и починен реальный (не тестовый) баг:
  `_RouteMatchScreenState.deactivate()` писал в `routeMatchAiModeProvider`
  синхронно — Riverpod запрещает менять provider во время финализации
  дерева виджетов, что и происходит при поп-е запушенного (не таб-корневого)
  инстанса. Раньше это никогда не срабатывало, т.к. экран жил только как
  таб-корень; резюм-флоу впервые сделал «поп через Navigator» реальным
  сценарием. Запись перенесена на microtask с проверкой `notifier.mounted`.
- **Локации на Главной.** Количество карточек уравнено с маршрутами —
  `.take(7)` на месте (раньше показывались все 20 из `homePlacesProvider`).
  Первое переключение на Локации показывало на мгновение всю Главную как
  один спиннер (`loading: () => Center(CircularProgressIndicator())`
  заменял целиком список вместе с шапкой/тумблером/поиском/чипами) —
  извлёк общий `_homeHeader()` и добавил `_buildLoadingList()`: шапка
  остаётся на месте, вместо карточек — `AppShimmer`+`AppSkeleton` (уже
  существующий переиспользуемый компонент, тот же паттерн, что в
  `profile_screen.dart`).
- Пересобран release APK на реальном API — `dist/CrimeaTrip-test-api.apk`
  (61 МБ), скопирован на Desktop.

`flutter analyze` чисто, `flutter test` 218/218 (+2 теста: back-навигация
из резюм-чата не улетает на Главную; Локации капается на 7 и не бликает
при первой загрузке).

### 2026-08-25 (2) — Место: серый фон Scaffold; настройки → «История чатов с ИИ»; release APK

- **Экран места, окончательный фикс полоски.** Причина всех предыдущих
  симптомов разом: `Scaffold.backgroundColor` на этом экране был
  `AppColors.pageSurface` (серый) вместо `elevatedSurface` (белый), в
  отличие от `RouteDetailsScreen`. Любой шов между белыми блоками (даже
  субпиксельный) показывал этот серый фон насквозь. Поменян на белый —
  теперь любой будущий шов сливается со сценой вместо просвечивания.
- **«История чатов с ИИ» в настройках** — было отложено (см. запись
  «Отложено» выше, `_startNewChat`/`onNewChat`), сейчас реализовано.
  Бэкенд уже имел всё нужное (`GET /route-builder/sessions`,
  `GET /route-builder/sessions/{id}/messages`, Phase 8B) — не хватало
  только мобильной части: `listSessions`/`listMessages` в
  `RouteMatchRepository`, экран `ChatHistoryScreen` (список прошлых
  сессий, label из `constraints`), новый route
  `route-match-resume` → `RouteMatchScreen(resumeSession: …)`, который
  открывается сразу в режиме чата и реплеит транскрипт. Реплей переиспользует
  тот же `_agentMessageFromResult`/блок-парсинг, что и живые ответы
  (`RoutePlanningMessageResult.fromJson` без изменений подходит и для
  сохранённых строк — поля просто отсутствуют и уже дефолтятся). Известное
  ограничение: у исторических `route_proposal_card` блоков нет вложенного
  `proposal` (только `proposal_id`), поэтому такие сообщения при реплее
  показываются текстом без интерактивной карточки — только для *прошлых*
  сообщений, не для текущего диалога.
- **Собран release APK** на реальном API (`scripts/build-signed-apk.sh`,
  `DATA_SOURCE=api`, `https://86-106-20-132.sslip.io`, бэкенд
  health-checked перед сборкой) — `dist/CrimeaTrip-test-api.apk` (61 МБ),
  скопирован на Desktop.

`flutter analyze` чисто, `flutter test` 216/216 (+3 теста на историю
чатов).

### 2026-08-25 — Экран места: хедер/фото фиксы; маршруты-слайдер: конечная колода + план рекомендаций

**Мобайл, экран Локации (`place_details_screen.dart` /
`collapsing_hero_header.dart`):**

- Фото при скролле «серело» и перезагружалось: `AppImages.coverImage`
  считал `memCacheWidth` **и** `memCacheHeight` из живых констрейнтов
  `LayoutBuilder`, а оба параметра входят в ключ картинки — на каждый
  пиксель скролла получался новый ключ → полный ре-декод. Заодно
  фон шапки (`CollapsingHeroSliver`) лежал в `SizedBox` внутри
  `Stack(fit: expand)`, которая сплющивала его высоту до текущей
  высоты шапки (проверено тестом на старом коде: слот ехал
  320→280→...→56). Фикс: decode-ширина только по ширине (с квантованием
  в бакеты по 256px, высота больше не передаётся), фон — `Positioned`
  с фиксированной `maxExtent`, не зависящей от текущей высоты шапки.
  Заодно чинит и «сплющенное» горизонтальное фото в вертикальных
  карточках слайдера маршрутов (`RouteHeroCard` → тот же `coverImage`,
  без второго измерения decode-размер больше не ломает пропорции).
- Скруглённая «губа» белого инфоблока была отдельным `CollapseLayer`
  со скейлом по умолчанию — не просто исчезала при скролле, а ещё и
  уменьшалась, отрываясь от краёв. Вынесена в общий переиспользуемый
  `CollapsingSheetLip` (используется и на экране Маршрута): полностью
  непрозрачная структурная деталь без fade/scale, анимируется только
  радиус (24→0).
- Постоянно включённый затемняющий градиент в шапке места (которого
  никогда не было на экране Маршрута) красился поверх «губы» и
  свёрнутого бара на любой стадии скролла, включая состояние покоя —
  давал видимый шов на стыке с инфоблоком и тонировал свёрнутый бар.
  Удалён (контраст иконок и так самодостаточен через стекло/чёрную
  подложку, как на экране Маршрута).
- Полоска была видна даже в состоянии покоя (без скролла): `heroHeight`
  места — дробное значение (`screenHeight * 0.57`, клампится в
  [430, 590]), в отличие от маршрута, где `expandedHeight = 320` —
  чистое целое. Дробная граница сливера давала волосяной зазор
  (субпиксельный шов, просвечивал серый фон Scaffold). Округлено до
  целого физического пикселя через `devicePixelRatioOf`.

**Мобайл, слайдер маршрутов (`route_swipe_deck.dart`,
`routes_catalog_screen.dart`):** свайп влево («пропустить») отправлял
карточку в конец колоды (`_deck.add(route)`) — колода была
бесконечной, реального «разбора» не происходило. Теперь пропуск
убирает карточку насовсем; уже избранные маршруты в колоду не попадают
(`favoriteRouteIdsProvider`); когда колода пуста — экран-заглушка
(иконка + «Вы разобрали все маршруты на сегодня» + кнопка «Открыть
список маршрутов» → `AllListScreen`). Полный план персонализации/
серверной ежедневной колоды (алгоритм, cron в 03:00 МСК, новые
эндпоинты) — [route-swipe-recommendations.md](route-swipe-recommendations.md),
бэкенд-часть не начата.

`flutter analyze` чисто, `flutter test` 213/213 (+5 новых тестов:
константность слота фото под скролл, кривая радиуса губы,
аспект-сохраняющее декодирование, конечность колоды + исключение
избранного).
...
- Зафиксировано [ADR-009](decisions/ADR-009-data-first-route-intelligence.md)
  и [route-intelligence-roadmap.md](route-intelligence-roadmap.md) (P0–P4,
