...
1. **Review-gate и публикация мест** — блокер эффекта P0: 5000
   импортированных мест остаются `draft`, а `place_picker` отбирает
   только `published`, поэтому генерация всё ещё видит 20 seed-мест.
   Нужен процесс проверки `proposed_slug` / `description` / `is_cover`.
