...
| Класс | Пример ответа |
| --- | --- |
| `greeting` | **не canned** — уходит в LLM с system rules (коротко, в роли помощника по маршрутам); под ответом — **динамические** action chips по `ask_field` |
...
Модель отвечает **структурированным JSON** (`assistant_text`, `ask_field`,
`action_ids`, `constraint_patch`). Backend валидирует allowlist и собирает
chips; свободный UI/HTML от модели запрещён.

**`confirmed_fields` (stated in chat):** список ключей параметров, которые
пользователь **явно сказал или нажал в этом чате** (чип, слайдер, toggle,
accept recommendation, или `constraint_patch` из его реплики). Это единственное,
что модель видит как «Известно».

**Черновик формы:** значения с экрана параметров до чата лежат в
`constraints`, но **не** попадают в `confirmed_fields`. ИИ получает их отдельно
как `form_draft_not_facts` (можно мягко предложить подтвердить, нельзя
утверждать как выбор). При первой фиксации интересов из чата черновой список
интересов формы **заменяется**, а не дописывается.

Полоску «Учли: …» в UI **не показываем** — это внутренняя семантика сессии.
...
LLM **не** ходит в БД напрямую: факты мест — PostGIS через backend
(generate / будущий ToolRegistry); RAG chunks — недоверенные DATA.
