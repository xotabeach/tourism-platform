...
chips; свободный UI/HTML от модели запрещён. В сессии `confirmed_fields`
отделяют явные ответы пользователя от draft-дефолтов формы — модель не
должна пересказывать неподтверждённый город/число людей.
