...
| Класс | Пример ответа |
| --- | --- |
| `greeting` | «Привет! У меня всё супер — я готов помочь составить маршрут по Крыму. Откуда стартуем и сколько у вас времени?» |
| `off_topic` | «Я могу помогать только с подбором маршрутов и мест в Крыму. Давайте вернёмся к поездке: город старта, длительность или интересы?» |
...
тем и запрет на инструменты вне ToolRegistry.
...
2. Off-topic и greeting дают типовые ответы, без вызова LLM при
   дешёвом локальном/серверном классификаторе (LLM только для on_topic).
