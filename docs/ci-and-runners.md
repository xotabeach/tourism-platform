...
**Грабля при создании переменной:** не ставить галочку **Mask variable**. GitLab
маскирует только значения от 8 символов, а `full` — четыре, и создание падает с
«The value must have 8 characters». Маскировать здесь и нечего: это флаг режима,
а не секрет. Protected — по желанию.

### Чтобы джобы шли именно на свой раннер

Раннер без тегов подхватывает untagged-джобы, но их же могут перехватить общие
раннеры GitLab — **и на практике перехватывают**: первый же пайплайн после снятия
ограничений 2026-09-02 ушёл на `k8s.saas-linux-small-amd64.runners-manager.gitlab.com`,
то есть в общий пул, хотя свой раннер был свободен. Наличие своего раннера само по
себе ничего не гарантирует.

Надёжный способ — отключить общие: GitLab → группа → Settings → CI/CD → Runners →
снять **Enable instance runners for this group**, и включить у своего раннера
**Run untagged jobs** (сделано 2026-09-02). Альтернатива — повесить раннеру тег и
проставить `tags:` на джобах, но тогда любая забытая джоба без тега зависнет
навсегда.

**Следствие, о котором надо помнить:** с отключёнными общими раннерами джобы теперь
некуда девать, если свой раннер лежит. Пайплайн не упадёт — он будет висеть в
`pending`. Если сборки застряли, первым делом проверять
`systemctl status gitlab-runner` на `201.24.55.130`.
